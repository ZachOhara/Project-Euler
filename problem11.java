public class problem11 {
   ...
   public static void main(String[] args) {
      ...
      int[] row3 = {81,49,31,73,55,79,14,29,93,71,40,67,53,88,30,3,49,13,36,65};
      ...
      int[][] table = {row1,row2,row3,row4,row5,row6,row7,row8,row9,row10,row11,row12,row13,row14,row15,row16,row17,row18,row19,row20};
      ...
      for(int x = 0; x < 19; x++)
      {
         for(int y = 0; y < 19; y++)
         {
            if (y+3 <= 19)
            {
               if (table[x][y] * table[x][y+1] * table[x][y+2] * table[x][y+3] > max)
               {
                  max = table[x][y] * table[x][y+1] * table[x][y+2] * table[x][y+3];
                  maxX = x;
                  maxY = y;
                  direction = "left";
               }
               if (x+3 <= 19)
               {
                  if (table[x][y] * table[x+1][y+1] * table[x+2][y+2] * table[x+3][y+3] > max)
                  {
                     max = table[x][y] * table[x+1][y+1] * table[x+2][y+2] * table[x+3][y+3];
                     maxX = x;
                     maxY = y;
                     direction = "down+left";
                  }
            ...
            if (x+3 <= 19)
            {
               if (table[x][y] * table[x+1][y] * table[x+2][y] * table[x+3][y] > max)
               {
                  max = table[x][y] * table[x+1][y] * table[x+2][y] * table[x+3][y];
                  maxX = x;
                  maxY = y;
                  direction = "down";
               }
